...
public class StandardBoard extends Board{
	...
	public int isCheck() {
		...
		{
			Pieces currentPiece = this.players[0].getPieces()[i];
			if(currentPiece.getX() != -1 && currentPiece.getY() != -1)
			{
				boolean[][] moveable_block = this.players[0].getPieces()[i].moveable(this);
	 ...
	 */
	public boolean checkMateCheck(Player player)
	{
		...
		for(int i = 0; i < 16; i++)
		{
			...
			for(int x = 0; x < 8; x++)
			{
				for(int y = 0; y < 8; y++)
				{
					...
					{
						if(this.movePiece(piece, x, y) == 0) result = false;
						break;
					}
				}
			}
	...
	/**
	 * 
	 * if one player's isLose status become 0;
	 * 
	 * @return game ends or not
	 */
	public boolean endGame()
	{
		if (draw == 0) return true; 
		if(this.players[0].getIsLose() == -1 && this.players[1].getIsLose() == 0) return false;//no one lost
		else if (this.players[0].getIsLose() == 0) return true;//player0 lost
		else return true;//player1 lost
	}
