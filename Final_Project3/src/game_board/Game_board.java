...
public class Game_board {
	public int max_Player_num;//4-8
	public int current_Player_num;//able_bodied player number
	...
	public Game_board(int num)
	{
		...
		this.Picked_Navigation = new Vector<Navigation>(20);
		
		for(int i = 0; i < this.max_Player_num; i++)
		{
			this.Players[i].cards_in_hand.add(Provision_Deck.remove(0));
		}
	}
	...
	public void Action()
	{
		if(this.turn == 0)
		{
			if(this.Provision_Deck.isEmpty())
			{
				next_round();
				return;
			}
			//deal provision cards
			Vector<Provisions> temp = new Vector<Provisions>(this.current_Player_num);
			...
			for(int j = 0; j < this.max_Player_num; j++)
			{
				if(this.Players[j].getCurrent_hp() > 0)//not fainted or dead
				{
					...
					{
						System.out.print(i + ". ");
						if(temp.get(i) instanceof Cash) System.out.print("Cash"+temp.get(i).getID());
						else System.out.print("Water"+temp.get(i).getID());
						System.out.print("\n");
					}*/
					//BufferedReader consoleIn = new BufferedReader(new InputStreamReader(System.in)); 
					//System.out.print(consoleIn.readLine());
		...
		{
			if(this.Picked_Navigation.isEmpty() && this.Navigation_Deck.isEmpty())
			{
				next_round();
				return;
			}
			...
			{
				(this.Navigation_Deck.remove(0)).resolve(this);
			}
			else
			{
				int index = 0;//use UI to let last player pick
				(this.Picked_Navigation.remove(index)).resolve(this);
	...
	private Vector<Navigation> initial_navigation_deck() {
		Vector<Navigation> result = new Vector<Navigation>(20);//change this number later
		result = initial_all_navigations();
		//using the Fisher¨CYates shuffle algorithm 
		for(int i = 0; i < 20; i++)
		{
			//int j = randInt(i, result.size()-1);
	...
	private Vector<Navigation> initial_all_navigations() {
		// TODO:manually set up all the navigation cards
		Vector<Navigation> result = new Vector<Navigation>(20);
		String[] name = new String[1];
		name[0] = "The Kid";
		for(int i = 0; i < 20; i++)
		{
			result.add(new Navigation(i, name, 1, name, true, true));
		}
		return result;
	}

	private Vector<Provisions> initial_provision_deck() {
		Vector<Provisions> result = new Vector<Provisions>(20);//change this number later
	...
	private Vector<Provisions> initial_all_provisions() {
		// TODO: manually set up all the provision cards
		Vector<Provisions> result = new Vector<Provisions>(20);//change this number later
		for(int i = 0; i < 10; i++)
		{
			result.add(new Water(i));
		}
		for(int i = 10; i < 20; i++)
		{
			result.add(new Cash(i));
		}
		return result;
	}
	...
	private void next_round()
	{
		...
		if(this.turn == 0 || this.turn == this.max_Player_num + 1)
		{
			this.curPlayer = this.Players[0];
		}
		else
		{
			this.curPlayer = this.Players[this.turn-1];
		}
